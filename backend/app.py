...
def find_answer(question):
    ...
    normalized_question = question.lower().replace(".", "").replace(",", "").replace("?", "").replace("!", "").replace(";", "").replace(":", "")
    ...
    for item in keywords:
        if any(word in normalized_question for word in item["words"]):
            return knowledge_base[item["key"]]
